...
void Graf::BFS(const string& nodeInicial, const string& nodeFinal, queue<string>& recorregut, stack<string>& cami)
{
	...
	visitat.resize(m_numNodes, false);
	queue<int> pendents;
	vector<stack<string>> camins;
	camins.resize(m_numNodes);

	int pos = distance(m_nodes.begin(), itNode);
	int posFinal = distance(m_nodes.begin(), itNodeFinal);

	visitat[pos] = true;
	pendents.push(pos);

	camins[pos].push(m_nodes[pos]);
	...
	while (!pendents.empty())
	{
		...
		if (nodeActual == posFinal)
		{
			cami = camins[posFinal];
			stack<string> camiInvertit;
			
			while (!cami.empty())
			{
				camiInvertit.push(cami.top());
				cami.pop();
			}
			cami = camiInvertit;
			return;
		...
		for (int col = 0; col < m_numNodes; col++)
		{
			if ((m_matriuAdj[nodeActual][col] != 0) && (!visitat[col]))
			{
				...
				camins[col] = camins[nodeActual]; // SUPER IMPORTANT: COPIAR TOT EL CAMI ANTERIOR
				camins[col].push(m_nodes[col]); // AÑADIMOS EL NUEVO NODO!!!
			}
		}
	}
}
...
int Graf::BFS(const string& nodeInicial, const string& nodeFinal, queue<string>& recorregut)
{
	...
	queue<int> pendents;
	vector<int> dist;
	dist.resize(m_numNodes, -1);

	if (itNode == m_nodes.end() || itNodeFinal == m_nodes.end())
		return -1;
